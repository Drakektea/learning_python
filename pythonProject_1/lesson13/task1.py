...
def create_matrix(rows_count, columns_count):
    return [[randint(-150, 150) for k in range(columns_count)] for j in range(rows_count)]


def add_matrix(first, second):
    return [[first[j][k] + second[j][k] for k in range(len(first[j]))] for j in range(len(first))]
...
print('sum of matrix_1 and matrix_2 =')
for row in matrix_3:
    print(*row)
print()
